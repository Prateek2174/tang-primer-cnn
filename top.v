...
//normalize int8
    ...
    .resize_wr_addr  (resize_wr_addr) //[13:0]
...
wire resize_rd_en;       //output -> ceb 96x96
    ...
    Gowin_SDPB resized_frame(
        ...
        .ada(resize_wr_addr),       //input [13:0] ada
...
// ================================================================
// UART FRAME SENDER -- reads the 96x96 resized frame buffer and
// streams it out over UART for data collection / training.
// Triggered once per camera frame on VSYNC falling edge.
// ================================================================
...
always @(posedge cam_pclk or negedge rst_n) begin
    if (!rst_n)
        ...
        vsync_prev_top <= cam_vsync;
end

wire frame_ready = vsync_prev_top & ~cam_vsync; // one-cycle pulse per frame
...
uart_frame_sender frame_sender(
    ...
    .frame_addr  (resize_addr),
    .frame_rd_en (resize_rd_en),
    .frame_dout  (resize_dout),
...
uart uart_tx(
    ...
    .tx_data_ready   (tx_data_ready), //data is ready to send
    .tx_pin          (uart_tx_pin), //serial data output
    .tx_busy         ()
);
