        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                FSM_GATHER: begin
                    ...
                    //same 1-cycle BSRAM latency pattern as mac_array's FSM_ADDR:
                    //skip the first cycle since map_c_dout doesn't correspond to
                    //this pixel_index's address yet

                    if (pixel_index > 0) begin
                        acc <= acc + map_c_dout;
                    end

                    if (pixel_index < CH_SIZE) begin
                        map_c_rd_en <= 1'b1;
